...
local function PerformAttackEntity( eyePos, target, lastSeenPos, bot, brain, move )
    ...
    if not hasClearShot then
        ...
        if not bot.lastHostilesTime or bot.lastHostilesTime < Shared.GetTime() - 45 then
        ...
        elseif dist < 10.0 and dist > 4.0 then
            -- too close, but not too close - back away while firing
    ...
    if doFire then
        ...
        -- always look at your target, idiot
...
kMarineBrainActions =
{
    ...
    CreateDefendAction( 0.1, function( pos, targetPos, bot, brain, move )
            if gBotDebug:Get("debugall") or brain.debug then
                DebugLine(bot:GetPlayer():GetEyePos(), targetPos+Vector(0,1,0), 0.0,     0,0,1,1, true)
            end
            PerformMove(pos, targetPos, bot, brain, move)
...
local function GetAttackUrgency(bot, mem)
    ...
    local passiveUrgencies =
    {
        ...
        [kMinimapBlipType.Infestation] = numOthers >= 2    and 0.1 or 0.15
    }
...
function CreateMarineBrainSenses()
    ...
    s:Add("nearestWeldable", function(db)
            ...
            local marine = db.bot:GetPlayer()
            local targets = GetEntitiesWithMixinForTeamWithinRange( "Weldable", kMarineTeamType, marine:GetOrigin(), 20.0 )
